...
// std::vector<std::vector<std::vector<posit>>> all_weights;

// std::vector<std::vector<posit>> weights_ih( num_input_neurons , std::vector<posit>(num_hidden_neurons));



// for ( int hid_idx = 0 ; hid_idx < num_hidden_neurons ; ++hid_idx ){
//     for ( int in_idx = 0 ; in_idx < num_input_neurons ; ++in_idx ){
//         weights_ih[in_idx][hid_idx] = posit(0,8,2);
//     }
// }
// all_weights.push_back(weights_ih);

// std::vector < std::vector<posit>> weights_ho ( num_hidden_neurons , std::vector<posit>(num_output_neurons));



// for (int out_idx = 0 ; out_idx < num_output_neurons ; ++out_idx ){
//     for ( int hi_idx = 0 ; hi_idx < num_hidden_neurons ; ++hi_idx ){
//         weights_ho[hi_idx][out_idx] = posit(0,8,2);
//     } 
// }
...
//             event_queue.push({t,INPUT_SPIKE_EVENT,-1,-1,0,neuron_idx,posit(0,8,2)});
//         }
//     }
// }
...
//                     posit effective_input_to_next_neuron = current_layer_weights[current_event.target_neuron_id][j];
...
int main() {
    posit a(0b00101001, 8, 2);
    posit b(0b10111001, 8, 2);
