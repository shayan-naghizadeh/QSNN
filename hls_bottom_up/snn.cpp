...
public:
    ...
    LIFNeuron() :
        membrane_potential(posit(0,8,2)), has_fired_in_current_step(false), last_update_time(0),
        threshold_voltage(posit(0,8,2)), leak_reversal_potential(posit(0,8,2)),
        fixed_leak_amount_per_step(posit(0,8,2)), current_reset_mechanism(RESET_TO_ZERO) {}

    LIFNeuron(posit v_th, posit e_l, posit fixed_leak_amt, ResetMechanism rest_mech = RESET_TO_ZERO) :
    ...
    posit get_membrane_potential() const { return membrane_potential; }
};
    ...
    LOAD_SPIKES: for (int t = 0; t < 50; ++t) {
        for (int n = 0; n < NUM_INPUT_NEURONS; ++n) {
            ...
            if (spike_input[t][n] == 1) {
                event_queue.push(SpikeEvent(t, INPUT_SPIKE_EVENT, -1, -1, 0, n, posit(0b01000000,8,2)));
            }
        }
    }
