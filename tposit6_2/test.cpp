...
int snn(
    ...
    posit6_2 v_threshold = posit6_2(0b001011,6,2);//////////////////////////////
    ...
    PROCESS_EVENTS: while (!event_queue.is_empty() && current_simulation_time <= MAX_SIMULATION_TIME) {
        ...
        if (current_simulation_time > MAX_SIMULATION_TIME) break;
        ...
        if (current_event.Type == INPUT_SPIKE_EVENT) {
            ...
            if (!fired_this_tick_input[target_id]) {
                ...
                if (fired) {
                    ...
                    input_spike_counts[target_id]++;   //    count input spikes
            ...
            if (target_layer == 1) {
                if (!fired_this_tick_hidden[target_id]) {
                    ...
                    if (fired) {
                        ...
                        hidden_spike_counts[target_id]++;  //   count hidden spikes
    ...
    SAVE_HIDDEN_POTENTIALS: for (int i = 0; i < NUM_HIDDEN_NEURONS; ++i) {
        #pragma HLS PIPELINE
        hidden_potentials[i] = all_layers_hidden[i].get_membrane_potential();
    }
    int max_spikes = -1;
